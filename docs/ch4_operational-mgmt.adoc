...
`oc describe` details how registries are configured and mapped to the `Azure Blob Storage` using the `REGISTRY_STORAGE` environment variables.
...
Select the _Resource Grou_* recently created and explore the *[Summary]* tabs. Next, on the right hand navigation panel, explore the *[Virtual Machines]*, *[Storage Accounts]*, *[Load Balancers]*, and *[Networks]* tabs
