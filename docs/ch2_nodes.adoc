...
The infrastructure nodes are just regular nodes but with different labels so they are only used to host the optional infrastructure components for *{ocp}* like the routers, registries, metrics or the aggregated logging to isolate those components of the regular applications.
