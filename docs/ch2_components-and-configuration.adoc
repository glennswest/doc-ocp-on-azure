...
As a best practice, instances deployed to run containers in *{ocp}* include a dedicated disk (`datadisk`) configured to store the container images as well as a dedicated disk configured to store the `emptyDir` volumes. The disk setup is provided in the `ARM` template of each virtual machine type like `master.json` in the git repository for *{ocp}* master instances.
